...
@include('adminPanel.tablePageTel.tableIndexTopLine')
...
@if($hasTablesInRooms > 0)
    ...
    <div style="width:100%;" class="d-flex flex-wrap justify-content-between">
        @foreach($tableRooms as $tableRoomOne)
            ...
            @else
                <a href="{{ route('dash.index', ['ri' => $tableRoomOne->id]) }}" class="btn btn-outline-dark shadow-none mb-1" style="width: 33%;">
                    <strong>{{$tableRoomOne->roomName}}</strong>    
                </a>    
            @endif
        @endforeach
...
<div id="allTablePageDiv" style="width:100%; background-color:whitesmoke;" class="d-flex flex-wrap justify-content-start mt-3">
    @php
        $sFor = Auth::user()->sFor;
