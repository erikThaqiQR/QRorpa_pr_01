                                ...
                                <div class="card mb-1 pr-1 pl-1" style="width: 100%;" id="tab{{$tabelOne->tableNr}}OrClDiv{{$activeClientsPNrOneNew}}">
                                    ...
                                    @foreach ($allPlates as $onePlateIdThisCl)
                                        ...
                                        <div class="card mb-1 mt-2 pl-1 pr-1" style="width: 100%; border:1px solid rgb(72,81,87);" id="tab{{$tabelOne->tableNr}}OrClDiv{{$activeClientsPNrOneNew}}Plate{{$onePlateIdThisCl}}">
                                            ...
                                            <?php
                                                $callTabOrders = TabOrder::query()
                                                            ->join('tab_verification_p_numbers as tvpn', 'tab_orders.id', '=', 'tvpn.tabOrderId')
                                                            ->where([
                                                                ['tab_orders.tabCode', $tabelOne->kaTab],
                                                                ['tvpn.phoneNr', $activeClientsPNrOneNew],
                                                                ['tab_orders.toPlate', $onePlateIdThisCl],
                                                            ])
                                                            ->select([
                                                                'tab_orders.*',
                                                ...
                                                $prodsInTabIdGroup = array();
                                                $prodsInTabGroupOnlyOneOrder = array();
                                                $prodsInTabSasiaGroup = array();
                                                $prodsInTabEmriGroup = array();
                                                $prodsInTabTableNrGroup = array();
                                                $prodsInTabPlateIdGroup = array();
                                                $prodsInTabQmimiGroup = array();
                                                $prodsInTabStatusConfirmGroup = array();
                                                $prodsInTabStatusCallGroup = array();

                                                $prodsInTabSasiaCalledGroup = array();
                                                $prodsInTabSasiaConfirmedGroup = array();

                                                foreach($callTabOrders as $oneTOrder){
                                                
                                                    if (in_array($oneTOrder->prodId, $prodsInTabIdGroup)) {
                                                        $prodsInTabGroupOnlyOneOrder[$oneTOrder->prodId] = 0;

                                                        $prodsInTabSasiaGroup[$oneTOrder->prodId] += $oneTOrder->OrderSasia;
                                                        $prodsInTabQmimiGroup[$oneTOrder->prodId] += number_format($oneTOrder->OrderQmimi, 2, '.', '');
                                                        if($prodsInTabStatusConfirmGroup[$oneTOrder->prodId]){
                                                            $prodsInTabStatusConfirmGroup[$oneTOrder->prodId] = $oneTOrder->status == 1;
                                                        }
                                                        if($prodsInTabStatusCallGroup[$oneTOrder->prodId]){
                                                            $prodsInTabStatusCallGroup[$oneTOrder->prodId] = $oneTOrder->abrufenStat == 1;
                                                        }

                                                        if($oneTOrder->abrufenStat == 1){
                                                            $prodsInTabSasiaCalledGroup[$oneTOrder->prodId] += $oneTOrder->OrderSasia;
                                                        }else if($oneTOrder->status == 1){
                                                            $prodsInTabSasiaConfirmedGroup[$oneTOrder->prodId] += $oneTOrder->OrderSasia;
                                                        }
                                                    }else{
                                                        array_push($prodsInTabIdGroup, $oneTOrder->prodId);
                                                        $prodsInTabGroupOnlyOneOrder[$oneTOrder->prodId] = $oneTOrder->id;

                                                        $prodsInTabSasiaGroup[$oneTOrder->prodId] = $oneTOrder->OrderSasia;
                                                        $prodsInTabEmriGroup[$oneTOrder->prodId] = $oneTOrder->OrderEmri;
                                                        $prodsInTabTableNrGroup[$oneTOrder->prodId] = $oneTOrder->tableNr;
                                                        $prodsInTabPlateIdGroup[$oneTOrder->prodId] = $oneTOrder->toPlate;
                                                        $prodsInTabStatusConfirmGroup[$oneTOrder->prodId] = $oneTOrder->status == 1;
                                                        $prodsInTabStatusCallGroup[$oneTOrder->prodId] = $oneTOrder->abrufenStat == 1;
                                                        $prodsInTabQmimiGroup[$oneTOrder->prodId] = number_format($oneTOrder->OrderQmimi, 2, '.', '');

                                                        $prodsInTabSasiaCalledGroup[$oneTOrder->prodId] = 0;
                                                        $prodsInTabSasiaConfirmedGroup[$oneTOrder->prodId] = 0;
                                            ...
                                            @foreach($prodsInTabIdGroup as $prodsInTabIdGroupOne)
                                            <!-- Display table order -->
                                                @if($prodsInTabGroupOnlyOneOrder[$prodsInTabIdGroupOne] != 0)
                                                    <?php
                                                        $thisTOrder = TabOrder::findOrFail($prodsInTabGroupOnlyOneOrder[$prodsInTabIdGroupOne]);   
                                                        $waLog = waiterActivityLog::where([['actType','newProdWa'],['actId',$thisTOrder->id]])->first();
                                                        if($waLog != Null ){ $waiterData = User::find($waLog->waiterId); }else{ $waiterData = Null; }
                                                        $time2D = explode(':',explode(' ',$thisTOrder->created_at)[1]);
                                                    ?>
                                                        ...
                                                        <p class="pl-1" style="width:60%; margin:0; padding-top:2px; overflow: hidden; white-space: nowrap; text-overflow: ellipsis; line-height:0.9;">
                                                            ...
                                                            @if ($thisTOrder->OrderType != 'empty' && $thisTOrder->OrderExtra != 'empty')
                                                                ...
                                                                <span style="font-size:0.6rem;">Type: {{$theTy2D[0]}} | 
                                                                    Extra:
                                                                    @foreach (explode('--0--',$thisTOrder->OrderExtra) as $exOne)
                                                                        @if($exOne != '')
                                                            ...
                                                            @elseif ($thisTOrder->OrderType != 'empty')
                                                                <?php $theTy2D = explode('||',$thisTOrder->OrderType); ?>
                                                                <br>
                                                                <span style="font-size:0.6rem;">Type: {{$theTy2D[0]}}</span>
                                                            @elseif ($thisTOrder->OrderExtra != 'empty')
                                                            ...
                                                            @if($thisTOrder->OrderKomenti)
                                                                <br>
                                                                <span style="font-size:0.6rem;"><strong>{{__('adminP.comment')}}: </strong><span style="color:red;">{{$thisTOrder->OrderKomenti}}</span></span>
                                                            @endif
                                                        </p>
...
<div class="modal" id="selectToPayProdsInSelectedGroup" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" data-backdrop="false" 
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-body d-flex flex-wrap justify-content-between">
                ...
                <button class="btn btn-outline-danger shadow-none mt-4" onclick="selectToPayProdsInSelectedGroupCancel()" style="width:100%; margin:0px;"><strong>X</strong></button>
            </div>
        </div>
    </div>
</div>  
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-body d-flex flex-wrap justify-content-between">
                ...
                <button class="btn btn-outline-danger shadow-none mt-4" onclick="selectPlateForAbrufenCancel()" style="width:100%; margin:0px;"><strong>X</strong></button>
            </div>
        </div>
    </div>
</div>  
