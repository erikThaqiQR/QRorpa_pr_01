<script>
    ...
    function buildTheNewProdInOrderTable(tNr, tOrId, tOrStatus, tOrQmimi, tOrCreAt, tOrSasia, tOrEmri, tOrPershkrimi, tOrType, tOrKoment, staffName, plateName, extrasAll, tOrSasiaDone, clPhNr, plateId, abrufenStat, prodId, tabCode){
        ...
        if ($('#tabOrderDivGroup'+prodId).length) {
            ...
            $('#tabOrderSasiaSpan'+prodId).text(newSasia);

            var oldQmimi = parseFloat($('#tabOrderQmimiSpan'+prodId).text());
            var newQmimi = oldQmimi + parseFloat(tOrQmimi);
            $('#tabOrderQmimiSpan'+prodId).text(newQmimi.toFixed(2));

        }else if ($('.tabOrderDivGroup'+prodId).length){

            var preSasia = parseInt($('.tabOrderSasiaSpan'+prodId).text());
            var preQmimi = parseFloat($('.tabOrderQmimiSpan'+prodId).text());

            $('.tabOrderDivGroup'+prodId).remove();
            ...
            newGroupOrderDiv =  '<div style="border: 1px solid rgb(72,81,87); border-radius:2px; margin-bottom:2px;" class="d-flex flex-wrap justify-content-between" id="tabOrderDivGroup'+prodId+'"'+
                                'onclick="openGroupProductSelectionPage(\''+prodId+'\',\''+tabCode+'\',\''+clPhNr+'\',\''+plateId+'\')">'+
                                    '<p class="pl-1" style="width:80%; margin:0; padding-top:2px; overflow: hidden; white-space: nowrap; text-overflow: ellipsis; line-height:0.9; font-weight:bold;">'+
                                        '<i class="fa-brands fa-osi"></i> <span id="tabOrderSasiaSpan'+prodId+'">'+newSasia+'x</span> '+tOrEmri+
                                    '</p>'+
                                    '<p style="width:20%; margin:0; text-align:center;"><strong><span id="tabOrderQmimiSpan'+prodId+'">'+parseFloat(newQmimi).toFixed(2)+'</span>.-</strong></p>'+
                                '</div>';
            ...
            tabOrDiv =  '<div style="border: 1px solid rgb(72,81,87); border-radius:2px; margin-bottom:2px;" id="tabOrderDiv'+tOrId+'"';
                        if(abrufenStat == 1){
            tabOrDiv += 'class="d-flex flex-wrap justify-content-between tabOrderDivGroup'+prodId+' tabOrderDivCalled"';
                        }else if(tOrStatus == 1){
            tabOrDiv += 'class="d-flex flex-wrap justify-content-between tabOrderDivGroup'+prodId+' tabOrderDivConfirmed"';                
                        }else{
            tabOrDiv += 'class="d-flex flex-wrap justify-content-between tabOrderDivGroup'+prodId+'"';                 
                        }
            tabOrDiv += 'onclick="closeOrSelect(\''+tNr+'\',\''+tOrId+'\',\''+tOrSasia+'\')">'+
                            '<p class="pl-1" style="width:60%; margin:0; padding-top:2px; overflow: hidden; white-space: nowrap; text-overflow: ellipsis; line-height:0.9;">'+
                                '<strong>'+
                                '<span id="tabOrderSasiaSpan'+tOrId+'" class="tabOrderSasiaSpan'+prodId+'">'+tOrSasia+'x</span> '+tOrEmri;
            ...
            tabOrDiv +=     '<p style="width:20%; margin:0; font-size:0.6rem; line-height:1.1; overflow: hidden; white-space: nowrap; text-overflow: ellipsis;">'+
                                ...
                                '</strong>'+
                            '</p>'+
                            '<p style="width:15%; margin:0; text-align:center;"><strong><span id="tabOrderQmimiSpan'+tOrId+'" class="tabOrderQmimiSpan'+prodId+'">'+parseFloat(tOrQmimi).toFixed(2)+'</span>.-</strong></p>';
