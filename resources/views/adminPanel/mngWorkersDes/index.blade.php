...
<div class="p-2 pb-5">
    ...
    <div class="d-flex justify-content-between">
        <div style="width: 100%;" class="d-flex justify-content-between">
            <h4 style="color:rgb(39,190,175); width:50%;"><strong>Kontrollieren Sie Ihre Arbeiter</strong></h4>
            <button style="width:50%;" class="btn btn-dark shadow-none" data-toggle="modal" data-target="#addWorker"><strong><i class="fa-solid fa-plus"></i> Fügen Sie einen Arbeiter hinzu</strong></button>
        </div>
    </div>
    ...
    <div class="d-flex flex-wrap justify-content-start" id="allWorkersWaiter">
        ...
        @foreach(User::where([['sFor',Auth::User()->sFor],['role','55']])->get()->sortByDesc('created_at') as $user)
            <div class="card mb-1" style="width: 19.5%; margin-right:0.5%;" id="oneUsers{{$user->id}}">
                ...
                <div class="card-body p-1">
                    <h5 class="card-title" style="margin-bottom:2px; font-size:1.4rem;">
                        <i data-toggle="modal" data-target="#deleteWorkerConf{{$user->id}}Modal" style="color:red;" class="fas fa-trash-alt mr-2"></i>
                        {{$user->name}}
                    </h5>
                    <p class="card-text" style="margin-bottom:2px; font-size:0.9rem;">{{$user->email}}</p>
                ...
                <button class="btn btn-outline-dark btn-block shadow-none" onclick="changePassForWoPrep('{{$user->id}}','{{$user->name}}')" data-toggle="modal" data-target="#changePassForWoModal">
                    <strong><i class="fa-solid fa-lock"></i> Änder das Passwort</strong>
                </button>
    ...
    <div class="d-flex flex-wrap justify-content-start" id="allWorkersCook">
        <h4 style="color:rgb(39,190,175); width:33%;">Ihre Köche</h4>
        <button style="width:66%;" class="btn btn-dark shadow-none" data-toggle="modal" data-target="#catToPlate"><strong> <i class="fa-solid fa-plate-wheat"></i> Legen Sie Kategorien auf Platten fest</strong></button>
        @foreach(User::where([['sFor',Auth::User()->sFor],['role','54']])->get()->sortByDesc('created_at') as $user)
            <div class="card mb-1 mt-1" style="width: 19.5%; margin-right:0.5%;" id="oneUsers{{$user->id}}">
                @if($user->profilePic != 'empty')
                <img class="card-img-top centerImg" src="storage/profilePics/{{$user->profilePic}}" alt="Card image cap">
                @else
                <img class="card-img-top centerImg" src="storage/icons/Asset 24800.png" alt="Card image cap">
...
@include('adminPanel.mngWorkersDes.infoVerModals')
...
<div id="AccessAndTableModalsForWaiters">
    @foreach(User::where([['sFor',Auth::User()->sFor],['role','55']])->get()->sortByDesc('created_at') as $worker)
        ...
        <div class="modal fade" id="setAccessForWo{{$worker->id}}Modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" data-backdrop="false" 
        style="background-color: rgba(0, 0, 0, 0.5); padding-top:5%;">
                    ...
                    <div class="modal-body">
                        <div class="d-flex flex-wrap justify-content-start" id="setAccessForWo{{$worker->id}}AControlls">
                            @foreach(accessControllForAdmins::where([['userId',Auth::User()->id],['accessDsc','!=','workersManagement'],['accessDsc','!=','Frei']])->get() as $oneAcControl)
                                ...
                                @if($hasAccess != NULL)
                                <button onclick="removeThisAccess('{{$hasAccess->id}}','{{$worker->id}}','{{$oneAcControl->id}}')" style="width:19.5%; margin-right:0.5%;" id="waiterAccess{{$worker->id}}O{{$oneAcControl->id}}"
                                class="btn btn-success mb-1 shadow-none"><strong>{{$oneAcControl->accessDsc}}</strong></button>
                                @else
                                <button onclick="addThisAccess('{{Auth::User()->id}}','{{$worker->id}}','{{$oneAcControl->id}}')"  style="width:19.5%; margin-right:0.5%;" id="waiterAccess{{$worker->id}}O{{$oneAcControl->id}}"
        ...
        <div class="modal fade" id="setTablesForWo{{$worker->id}}Modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" data-backdrop="false" 
        style="background-color: rgba(0, 0, 0, 0.5); padding-top:5%;">
            <div class="modal-dialog modal-xl" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                    ...
                    <div class="modal-body">
                        <div class="d-flex flex-wrap justify-content-start" id="setTablesForWo{{$worker->id}}AControlls">
                            @foreach(TableQrcode::where('Restaurant',Auth::User()->sFor)->get() as $oneTable)
                                ...
                                @if($tableAccess != NULL)
                                <button onclick="removeThisTable('{{$tableAccess->id}}','{{$worker->id}}','{{$oneTable->tableNr}}')" style="width:8%; margin-right:0.33%;" id="waiterTable{{$worker->id}}O{{$oneTable->tableNr}}"
                                ...
                                style="width:8%; margin-right:0.33%;" class="btn btn-outline-success mb-1 shadow-none">{{$oneTable->tableNr}}</button>
                                @endif
                            @endforeach 
                        </div>
                    </div>
                                ...
                                <div style="width:100%;" class="mb-3">
                                        ...
                                        <div style="cursor: pointer; position:relative; object-fit: cover;" >
                                            ...
                                            @if(strlen($kat->emri) > 20)
                                                <div class="teksti d-flex justify-content-between" style="font-size:20px;  margin-bottom:13px;">          
                                                    ...
                                                    @if($prodKate != NULL)
                                                    <div style="width: 8%;" class="text-center" id="KategoriPlus{{$kat->id}}O{{$worker->id}}">
                                                    ...
                                                    <span style="width: 70%;" class="text-left" onclick="showProKat('{{$kat->id}}','{{$worker->id}}')"><strong>{{$kat->emri}}</strong></span> 
                                                    <button style="width: 20%;" class="btn btn-dark mr-3" data-toggle="modal" data-target="#categoryTAndE{{$kat->id}}O{{$worker->id}}">T / E</button>
                                                </div>
                                            @else
                                    ...
                                    <div class="container prodsFoto" id="prodsKatFoto{{$kat->id}}O{{$worker->id}}" style="display:none;">
                                        @foreach(Produktet::where('toRes', '=', Auth::user()->sFor)->where('kategoria','=',$kat->id)->get()->sortByDesc('visits') as $ketoProd)
                                            <div class="row p-2" id="catProds{{$ketoProd->id}}O{{$worker->id}}">
                                                        ...
                                                        <div class="col-12 product-section">
                                                            <div class="row">
                                                                ...
                                                                <div class="col-3 add-plus-section">
                                                                    ...
                                                                    class="btn-block btn btn-success" id="addProductForCookBtn{{$worker->id}}O{{$ketoProd->id}}">Registrieren</button>
                                                                    @else
                                                                    <button onclick="addProductForCook('{{$worker->id}}','{{$ketoProd->id}}','{{$kat->id}}')" 
                                                                    class="btn-block btn btn-outline-success" id="addProductForCookBtn{{$worker->id}}O{{$ketoProd->id}}">Registrieren</button>
                                                                    @endif
                                                                </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="d-flex justify-content-between mb-3">
                        <button class="btn btn-outline-dark shadow-none" id="woType55" onclick="selectWorkerType('55')" style="width:33.2%;"><strong>Kellner</strong></button>
                        <button class="btn btn-outline-dark shadow-none" id="woType54" onclick="selectWorkerType('54')"  style="width:33.2%;"><strong>Koch</strong></button>
                        <button class="btn btn-outline-dark shadow-none" id="woType53" onclick="selectWorkerType('53')"  style="width:33.2%;"><strong>Buchhalter</strong></button>
                    </div>
                    ...
                    <div class="d-flex flex-wrap justify-content-between mb-2" style="max-height:145px; overflow-y:scroll;" id="catToPlateShowPlates">
                        ...
                        @else
                            @foreach (resPlates::where('toRes',Auth::user()->sFor)->get() as $onePlate)
                                <div class="p-2 mb-1 text-center d-flex" style="width: 49%; border:1px solid rgb(72,81,87); border-radius:4px;">
                            ...
                            <div style="width:100%;" class="mb-2">
                                <div class="allKatFotoPlate" id="catDivPlate{{$kat->id}}">
                                    <div style="cursor: pointer; position:relative; object-fit: cover;" >
                                        ...
                                        @if(strlen($kat->emri) > 20)
                                            <div class="teksti d-flex justify-content-around" style="font-size:20px; margin-bottom:13px;">          
                                                <span style="width: 60%;" class="text-left"><strong>{{$kat->emri}}</strong></span> 
                                                <div class="d-flex flex-wrap justify-content-start" style="width: 39%; margin-top:-50px; margin-right:5%; max-height:80px; overflow-y:scroll;">
                                                    @foreach (resPlates::where('toRes',Auth::user()->sFor)->get() as $onePlate)
                                                        <button id="p{{$onePlate->desc2C}}Btn{{$kat->id}}" onclick="selCatPlate('{{$onePlate->desc2C}}','{{$kat->id}}')" style="width: 33%; margin-right:0.33%;" 
...
@include('adminPanel.mngWorkersDes.mngWorkersDesJS')