        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer d-flex justify-content-between">
                    <button type="button" id="clDaysToPayModalBtn1" style="width:48%; margin:0px;" class="btn btn-dark shadow-none" data-dismiss="modal">Stornieren</button>
                    <button type="button" id="clDaysToPayModalBtn2" style="width:48%; margin:0px;" class="btn btn-success shadow-none" onclick="clDaysToPay()">Sparen</button>
                </div>
        ...
        <div id="notConfirmedBillsDiv">
            ...
            @foreach ($notConfirmed as $oneRecNot)
                ...
                @endif
                    ...
                    ?>
                    <p class="text-center" style="width:33%; font-weight:bold;"># {{$refId}}</p>
                    <p class="text-center" style="width:33%; font-weight:bold;">{{$totShuma}} CHF</p>
                    <p class="text-center" style="width:33%; font-weight:bold;">{{$oneRecNot->clientFirma}}</p>
                    ...
                    <p class="text-center" style="width:49.5%; font-weight:bold; margin-top:-15px; margin-bottom:0px;">{{$stD2d[2]}}-{{$stD2d[1]}}-{{$stD2d[0]}}</p>
                    <p class="text-center" style="width:49.5%; font-weight:bold; margin-top:-15px; margin-bottom:0px;">({{$oneRecNot->clientDaysToPay}} Tage) {{$endD}}</p>
            ...
            <div id="confirmedBillsList">
                @foreach ($confirmed as $oneRecNot)
                    ...
                    <div class="d-flex flex-wrap justify-content-between pt-1 pb-1 mb-2" style="background-color:rgba(40,167,69,0.2); border-radius:4px;">
                        ...
                        <p class="text-center" style="width:49.5%; font-weight:bold; margin-top:-15px; margin-bottom:0px;">{{$stD2d[2]}}-{{$stD2d[1]}}-{{$stD2d[0]}}</p>
                        <p class="text-center" style="width:49.5%; font-weight:bold; margin-top:-15px; margin-bottom:0px;">{{$endD}}</p>
                        ...
                        @if($tM != NULL)
                        <p class="text-center" style="width:49.5%; font-weight:bold; margin-bottom:0px;">Von {{$tM->name}}</p>
                        @else
                        <p class="text-center" style="width:49.5%; font-weight:bold; margin-bottom:0px;">Von ---</p>
                        @endif

                        @if ($oneRecNot->type == 'order')
                            <form style="width:49.5%" method="post" action="{{ route('emailBill.rechnungGetBilMngPage') }}">
                                {{csrf_field()}}         
                                <input type="hidden" value="{{$oneRecNot->id}}" name="emBiId">
                                <button type="submit" class="btn btn-info shadow-none" style="width:100%"> <i class="fa fa-file-pdf-o" aria-hidden="true"></i></button>
                            </form>
                        @else
                            <div style="width:49.5%" class="d-flex justify-content-between align-items-center">
                                <form style="width:49.5%" method="post" action="{{ route('giftCard.giftCardGetReceipt') }}">
                                    {{csrf_field()}}         
                                    <input  id="giftCardId" type="hidden" value="{{$oneRecNot->giftCardId}}" name="giftCardId">
                                    <button type="submit" class="btn btn-info shadow-none" style="width:100%;"> <i class="fa fa-file-pdf-o" aria-hidden="true"></i></button>
                                </form>
                                <a style="width: 49.5%;" class="btn btn-secondary mt-1 ml-1 shadow-none" href="{{ route('giftCard.rechnungDownloadGC', $oneRecNot->giftCardId) }}">
                                    <i class="fa-solid fa-receipt"></i> <strong></strong>
                                </a>
                            </div>
                        @endif
            ...
            @foreach (emailReceiptFromAdm::where([['forRes',Auth::user()->sFor],['statusConf','0']])->orderByDesc('created_at')->get() as $oneRecNot)
                @if(rechnungClientToBills::where('billId',$oneRecNot->id)->first() == Null)
                    ...
                    @endif
                        ...
                        ?>
                        <p class="text-center" style="width:33%; font-weight:bold;"># {{$thisOrderAR->refId}}</p>
                        <p class="text-center" style="width:33%; font-weight:bold;">{{number_format($totShuma, 2, '.', '')}} CHF</p>
                        <p class="text-center" style="width:33%; font-weight:bold;">{{$oneRecNot->exInfoFirma}}</p>
                        ...
                        <p class="text-center" style="width:49.5%; font-weight:bold; margin-top:-15px; margin-bottom:0px;">{{$stD2d[2]}}-{{$stD2d[1]}}-{{$stD2d[0]}}</p>
                        <p class="text-center" style="width:49.5%; font-weight:bold; margin-top:-15px; margin-bottom:0px;">({{$oneRecNot->exInfoDaysToPay}} Tage) {{$endD}}</p>
                ...
                @foreach (emailReceiptFromAdm::where([['forRes',Auth::user()->sFor],['statusConf','9']])->orderByDesc('created_at')->get() as $oneRec)
                    @if(rechnungClientToBills::where('billId',$oneRec->id)->first() == Null)
                        ...
                        <div class="d-flex flex-wrap justify-content-between pt-1 pb-1 mb-2" style="background-color:rgba(40,167,69,0.2); border-radius:4px;">
                            ...
                            <p class="text-center" style="width:33%; font-weight:bold;"># {{$thisOrderAR->refId}}</p>
                            <p class="text-center" style="width:33%; font-weight:bold;">{{number_format($totShuma, 2, '.', '')}} CHF</p>
                            <p class="text-center" style="width:33%; font-weight:bold;">{{$oneRec->exInfoFirma}}</p>
