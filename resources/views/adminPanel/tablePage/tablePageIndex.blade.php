...
use App\restorantTablesToRoom;
    ...
    $allTables = TableQrcode::where('Restaurant',Auth::user()->sFor)->orderBy('tableNr')->get();
...
@include('adminPanel.tablePage.tableIndexTopLine')
...
@if($hasTablesInRooms > 0)
    <?php
        ...
        if(isset($_GET['ri'])){
            $ri = $_GET['ri'];
            foreach(restorantTablesToRoom::where('toRoomId',$ri)->get() as $tableRoomOne){ 
                array_push($tableIdToShow,$tableRoomOne->tableId);
            }
        }else{
            $ri = 0;
            foreach(restorantTablesToRoom::where([['toRes',Auth::user()->sFor],['toRoomNr','1']])->get() as $tableRoomOne){ 
                array_push($tableIdToShow,$tableRoomOne->tableId);
            }
        }
    ?>
    <div style="width:100%;" class="d-flex justify-content-between">
        @foreach($tableRooms as $tableRoomOne)
            ...
            @else
                <a href="{{ route('dash.index', ['ri' => $tableRoomOne->id]) }}" class="btn btn-outline-dark shadow-none mb-1" style="width: {{$tableRoomBtnWidth}}%;">
                    <strong>{{$tableRoomOne->roomName}}</strong>    
                </a>    
            @endif
...
@include('adminPanel.tablePage.tablePageIndexScript')