            ...
            <div class="modal-header split-the-bill-initiate-header">
                ...
                <div class="split-bill-header-pills">
                    <span class="split-bill-pill">
                        ...
                        <span class="split-bill-pill-text"><span id="splitTheBillInitiateModalClientsNr">x</span></span>
                    </span>
                    <span class="split-bill-pill">
                        <svg class="split-bill-pill-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
...
<script>
    ...
    function splitBillpayAllProdsCash(){
        $.ajax({
			...
			success: (respo) => {
                ...
                $('#splitBillClientDiv1'+respo2D[1]).attr('class','d-flex justify-content-between pt-2 alert-success');
    ...
    function splitBillOpenGCPay(clientNr, tableNr, clientsNr){
        var newDiv1_2 = '<button class="btn btn-danger shadow-none" style="width:20%;" onclick="splitBillCloseGCPay(\''+clientNr+'\', \''+tableNr+'\', \''+clientsNr+'\')"><strong><i class="fa-solid fa-arrow-left"></i> Zurück</strong></button>'+
				...
				success: (response) => {
                    ...
                    if(response == 'gcNotFound'){
                        ...
                        var newDiv1_2 = '<button class="btn btn-danger shadow-none" style="width:20%;" onclick="splitBillCloseGCPay(\''+clientNr+'\', \''+tableNr+'\', \''+clientsNr+'\')"><strong><i class="fa-solid fa-arrow-left"></i> Zurück</strong></button>'+
                                        '<div style="width:75%;" class="input-group">'+
                                            '<div class="input-group-prepend">'+
                                                '<span class="input-group-text" id="basic-addon1">Restbetrag: '+parseFloat(respo2D[1]).toFixed(2)+' CHF</span>'+
                                            '</div>'+
...
#splitTheBillInitiateModal .split-bill-client-card-head {
...
#splitTheBillInitiateModal .split-bill-client-card-body-2 .alert-success {
    background-color: #d4edda !important;
    color: #155724 !important;
    border-radius: 0;
}
#splitTheBillInitiateModal .split-bill-client-card-head .alert-success .split-bill-client-head strong,
