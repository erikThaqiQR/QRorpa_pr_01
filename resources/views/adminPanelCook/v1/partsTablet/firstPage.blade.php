                            ...
                            @if($theProd->kategoria == $oneCat && !in_array($oneTP->prodId,$productsBllock) )
                                ...
                                @foreach($allActConfOrders as $oneTPIn)
                                    @if($oneTPIn->prodId == $oneTP->prodId && $oneTPIn->OrderSasia > $oneTPIn->OrderSasiaDone)
                                        ...
                                        <p class="proColDetailsBottom d-flex">
                                            ...
                                            @else
                                            <button type="button" onclick="removeOneDone('{{$oneTPIn->id}}','{{$oneTP->prodId}}')" class="btn btn-danger p-1" style="width: 50%;" id="removeOneDoneBtn{{$oneTPIn->id}}">
                                                <strong>-</strong>
                                            </button>
                                            @endif

                                            <button type="button" onclick="addOneDone('{{$oneTPIn->id}}','{{$oneTP->prodId}}')" class="btn btn-success p-1" style="width: 50%;" id="addOneDoneBtn{{$oneTPIn->id}}">
                                                <strong>+</strong>
                                            </button>
                                        </p>
                                ...
                                @if (count($finishedOrThisPr) > 0)
                                    ...
                                    @foreach($finishedOrThisPr as $oneTPIn)
                                        ...
                                        <p class="proColDetailsBottom d-flex">
                                            <button style="width:50%;" class=" btn btn-warning p-1" disabled> <i class="far fa-check-circle"></i></i>
                                            </button>
                                            <span class="pt-1" style="width: 50%;"><strong>T:{{$oneTPIn->tableNr}}</strong></span>
                                        </p>
