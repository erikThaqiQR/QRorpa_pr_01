        ...
        <div class="swiper-container p-0" style="background-color:rgb(39,190,175) ;">
            <div class="swiper-wrapper p-2" style="background-color:rgb(39,190,175) ;">
                ...
                @foreach($allTables as $rTable)
                    <?php
                        $tableShow = False;
                        foreach(TabOrder::where([['tabCode','!=','0'],['toRes',Auth::User()->sFor],['status','1'],['tableNr',$rTable]])->get() as $checkTToShowT){
                    ...
                    <div class="swiper-slide d-flex flex-wrap" style="background-color:transparent; border-radius:6px; width:100%; flex-direction: row" id="TableColumnCookTO{{$rTable}}">
                        ...
                        <?php
                            $allPlates = array();
                            foreach(TabOrder::where([['tabCode','!=','0'],['toRes',Auth::User()->sFor],['status','1'],['tableNr',$rTable],['toPlate','!=','0']])->get() as $oTOrThisT){
                        ...
                        @foreach ($allPlates as $plateOne)
                            <?php 
                                ...
                                $nrAll = (int)0;
                                $tabOrCalled = 0;
                                foreach(TabOrder::where([['tabCode','!=','0'],['toRes',Auth::User()->sFor],['status','1'],['tableNr',(int)$rTable],['abrufenStat','1'],['toPlate',$thePl->id]])->get() as $toOneCnt){
                            ...
                            ?>
                            @if( $tabOrCalled > 0 )
                            <div class="text-center pt-2 pb-2 mb-2 allPlate{{$rTable}} cookDivGlowCalled" style="width:100%; border-radius:6px; background-color:rgba(191,191,191,255); margin-bottom:0px; " 
                            ...
                            @endif
                                ...
                                @endif
                                    ...
                                    <div style="width: 100%;" id="ordersListT{{$rTable}}P{{$plateOne}}">
                                        @foreach(TabOrder::where([['tabCode','!=','0'],['toRes',Auth::User()->sFor],['status','1'],['tableNr',(int)$rTable],['toPlate','!=','0']])->get() as $oTOrTP)
                                            @if($oTOrTP->OrderSasia != $oTOrTP->OrderSasiaDone)
                                                    ...
                                                    @if ( $thePlaOr->desc2C == $plateOne && $tp2 != Null)
                                                        ...
                                                        @if ($has2nAccess == 1)
                                                            ...
                                                            <div onclick="changeStatOrCookYes('{{$rTable}}','{{$plateOne}}','{{$oTOrTP->id}}','{{$sas}}')"
                                                            class="prodOneT{{$rTable}}P{{$plateOne}} text-left justify-content-between flex-wrap crsPointer mb-1 p-1 pt-2 pb-2" 
                                                            style="width: 100%; background-color:rgb(208, 250, 207); display:none;"
                        ...
                        <?php 
                            ...
                            $nrAll = (int)0;
                            $tabOrCalled = 0;
                            foreach(TabOrder::where([['tabCode','!=','0'],['toRes',Auth::User()->sFor],['status','1'],['tableNr',(int)$rTable],['abrufenStat','1'],['toPlate',0]])->get() as $toOneCnt){
                                if($toOneCnt->OrderSasia != $toOneCnt->OrderSasiaDone){ $tabOrCalled++; break; }
                            }
                        ?>
                            ...
                            @endif
                                ...
                                <div style="width: 100%;" id="ordersListT{{$rTable}}P0">
                                    @foreach(TabOrder::where([['tabCode','!=','0'],['toRes',Auth::User()->sFor],['status','1'],['tableNr',(int)$rTable],['toPlate','0']])->get() as $oTOrTP)
                                        @if($oTOrTP->OrderSasia != $oTOrTP->OrderSasiaDone)
                                            ...
                                            @if($tp2 != NULL)
                                                ...
                                                @if($tc2 != NULL && $tc2->forPlate == 0)
                                                    ...
                                                    @if ($has2nAccess == 1)
                                                        ...
                                                        @if($oTOrTP->abrufenStat == 1)
                                                        <div onclick="changeStatOrCookYes('{{$rTable}}','0','{{$oTOrTP->id}}','{{$sas}}')" 
                                                        class="prodOneT{{$rTable}}P0 text-left justify-content-between flex-wrap crsPointer mb-1 p-1 pt-2 pb-2" 
                                    ...
                                    <script>
                                        $('#nrAllT{{$rTable}}P0').html('{{$nrAll}}');    
                                        $('#nrDoneT{{$rTable}}P0').html('{{$nrDone}}');    
                                    </script>
                                    @if ($nrAll == $nrDone)
                                        <script>
