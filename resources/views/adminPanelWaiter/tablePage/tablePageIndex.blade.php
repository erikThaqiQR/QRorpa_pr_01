...
@include('adminPanelWaiter.tablePage.tableIndexTopLine')
...
@if($hasTablesInRooms > 0)
    <?php
        ...
        $nrOfTableRooms = restorantTableRoom::where('toRes',Auth::user()->sFor)->count();
        $tableRoomBtnWidth = 98 / $nrOfTableRooms;
    ...
    <div style="width:100%;" class="d-flex justify-content-between">
        @foreach($tableRooms as $tableRoomOne)
            @if(($ri == 0 && $tableRoomOne->roomNumber == '1') || $ri == $tableRoomOne->id)
                <button type="button" class="btn btn-dark shadow-none mb-1" style="width: {{$tableRoomBtnWidth}}%;">
                    <strong>{{$tableRoomOne->roomName}}</strong> 
                </button>
            @else
                <a href="{{ route('admWoMng.indexAdmMngPageWaiter', ['ri' => $tableRoomOne->id]) }}" class="btn btn-outline-dark shadow-none mb-1" style="width: {{$tableRoomBtnWidth}}%;">
                    <strong>{{$tableRoomOne->roomName}}</strong>    
                </a>    
            @endif
...
<div id="allTablePageDiv" style="width:100%;" class="d-flex flex-wrap justify-content-start pb-4 mt-3">
    ...
    @else
        @foreach($showTablesIns as $tabelOne)
            <div id="tableIconDiv{{$tabelOne->tableNr}}" class="allTablesDes mb-2" style="width:5.5%; margin-right:0.821%; margin-left:0.821%;">
                ...
                @endif
                <input type="hidden" id="tableModalForReaload{{$tabelOne->tableNr}}" value="0">
            </div>
        @endforeach
    @endif
...
@include('adminPanelWaiter.tablePage.newOrderRegEl')
@include('adminPanelWaiter.tablePage.tableIndexActiveOrMd')
@include('adminPanelWaiter.tablePage.tableIndexNotifications')
@include('adminPanelWaiter.tablePage.tablePageIndexScript')