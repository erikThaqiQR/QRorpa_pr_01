<?php
  ...
  $svgObjAllK = svgTableMapData::where([['toRes','69'],['toRoom','12'],['formShape','k']])->get();
  $svgObjAllRR = svgTableMapData::where([['toRes','69'],['toRoom','12'],['formShape','rr']])->get();
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="{{$svgBoxSize1}} {{$svgBoxSize2}} {{$svgBoxSize3}} {{$svgBoxSize4}}">
  ...
  <rect x="673.55" y="216.672" width="88.066" height="26.723" style="stroke: rgb(0, 0, 0); fill: rgb(172, 236, 106); stroke-width: 0px;" id="objekt01"/>
    ...
    <g id="tableIconDiv{{$svgObjOne->tableNr}}">
      ...
      @if($tabelOne->kaTab != 0 )
        @if($tabelOne->kaTab == -1)
          ...
          <text onclick="setTabStat('{{$tabelOne->tableNr}}','{{$tabelOne->id}}','0')"
          style="white-space: pre; fill: rgb(51, 51, 51); font-family: Arial, sans-serif; font-size: 18px;" 
          x="{{$xtableNr}}" y="{{$ytableNr}}">Lounge</text>
        @elseif(TabOrder::where([['tabCode',$tabelOne->kaTab],['status',0]])->count() > 0)
