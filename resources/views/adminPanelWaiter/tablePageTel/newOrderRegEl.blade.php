...
<!-- new tab order Modal -->
<div class="modal" id="newTabOrderModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" data-backdrop="false" 
    ...
    <div class="modal-dialog modal-xl" role="document" style="margin-top: 5px;">
        <div class="modal-content">
            <div class="modal-body">
                ...
                <div class="pt-2 pb-5" style="padding-left:2px; padding-right:2px;">
                        ...
                        <div id="addOrForTAModalBody2Left" style="width:100%">
                            <?php 
                                ...
                                foreach($catsCall  as $kat){
                                    if($stepCatsArray == 'A'){
                                        array_push($catsArrayP1,$kat);
                                        $stepCatsArray = 'B';
                                    }else{
                                        array_push($catsArrayP2,$kat);
                                        $stepCatsArray = 'A';
                                    }
                                }
                            ?>
                            ...
                            <div class="swiper-container p-0" style="width:100%; border:1px solid rgb(39,190,175); border-radius:6px; overflow: hidden;">
                                <div id="addOrForTAModalBody2LeftCats" class="swiper-wrapper p-2"> 
                                    @foreach(range(0, count($catsArrayP1)-1) as $nrRnd)
                                        <?php
                                            if(isset($catsArrayP1[$nrRnd])){ $kat = $catsArrayP1[$nrRnd]; }else{ $kat = 'none';}
                                            if(isset($catsArrayP2[$nrRnd])){ $kat2 = $catsArrayP2[$nrRnd]; }else{ $kat2 = 'none';}
                                        ?>
                                        <div class="swiper-slide mr-1" style=" width:64px; height:85px;">
                                            @if($kat != 'none')
                                                <img class="{{ $catShowFirst == 0 ? 'newProdCatSelected' : '' }}" id="taNewProdCatImg{{$kat->id}}"
                                                style="border-radius:5px; width:64px; height:40px; margin-bottom:5px;" src="storage/kategoriaUpload/{{$kat->foto}}"alt=""
                                            ...
                                            @if($kat2 != 'none')
                                                <img class="mr-1 swiper-slide {{ $catShowFirst == 0 ? 'newProdCatSelected' : '' }}" id="taNewProdCatImg{{$kat2->id}}"
                                                style="border-radius:5px; width:64px; height:40px;" src="storage/kategoriaUpload/{{$kat2->foto}}"alt=""
                                                onclick="showCatNewTabOrderModal('{{$kat2->id}}')">
                                                <?php 
                                                    if($catShowFirst == 0){ $catShowFirst = $kat2->id; }
                                                    if(!in_array($kat2->id,$taCats)){ array_push($taCats,$kat2->id); }    
                                                ?>
                                                <script>
                                                    if(openCatTaNewPro == 0){ openCatTaNewPro = '{{$kat2->id}}'; }
                                                </script>
                                            @endif
                                        </div>
                                    @endforeach
                            ...
                            @foreach ($taCats as $taCatsOneId)
                                ...
                                @endif
                                    ...
                                    @foreach($theProdsArr as $ketoProd) 
                                        <div class="d-flex justify-content-between" style="width:49.9%; margin: 4px 0.05% 4px 0.05%; border:1px solid rgb(39,190,175); border-radius:0.25rem;">
                                            ...
                                            @else
                                            <button onclick="checkInOpenTypeSelect('{{$ketoProd->id}}')" class="btn shadow-none" id="addToCartExpressBtn{{$ketoProd->id}}" style="width:20%; height:100%; margin:0px; padding:1px;">
                                            @endif
        ...
        <div class="modal-content" style="border-radius:30px;">
            <div class="modal-body d-flex flex-wrap justify-cntent-between" id="ProdNewDtlBody">
                ...
                <div style="width: 100%;" class="form-group mt-2 mb-2">
                    <textarea placeholder="{{__('adminP.comment')}}..." name="koment" style="font-size:16px; z-index:9999;"
                ...
                width:95%; bottom:50px; right:2.5%; left:2.5%; font-size:22px;" id="sendOrderBtn"
