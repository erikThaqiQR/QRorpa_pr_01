...
class OnlinePayFromStafController extends Controller
{
 
    public function oPayFStafOpenOrder(){return view('onlPayClPage');}
    public function onlPayClPageFinish(){return view('onlPayClPageFinish');}
    ...
    public function OnlinePayInitiate(Request $req){
        $newOPStaf = new onlinePayQRCStaf();
        $newOPStaf->resId = $req->resId;
        $newOPStaf->tableNr = $req->tableNr;
        ...
        $newOPStaf->tvsh = $req->tvsh;
        $newOPStaf->thePayM = "Online";
        $newOPStaf->payType = "payAll";
        $newOPStaf->resComment = $req->resComment;
        $newOPStaf->save();


        $word = array_merge(range('a', 'z'), range('A', 'Z'), range('0', '9'), range('A', 'Z'));
        shuffle($word);
    ...
    public function OnlinePayInitiateSelective(Request $req){
        $newOPStaf = new onlinePayQRCStaf();
        $newOPStaf->resId = $req->resId;
        $newOPStaf->tableNr = $req->tableNr;
        $newOPStaf->qrCodeTP = "none";
        ...
        $newOPStaf->giftCardId = $req->discGCId;
        $newOPStaf->tvsh = $req->tvsh;
        $newOPStaf->prodSelected = $req->selProds;
        $newOPStaf->thePayM = "Online";
        $newOPStaf->payType = "paySelected";
        $newOPStaf->resComment = $req->resComment;
        $newOPStaf->save();
    ...
    public function oPayFStafPayAllRes(Request $req){
        ...
        $payload = array(
            ...
            'ReturnUrls' => array(
                'Success' => "https://demo.qrorpa.ch/oPayFStafPayAllResRegOrd/?OPR=".$OnlinePayRegister2,
    ...
    public function oPayFStafPayAllResRegOrd(Request $request){
        ...
        $newOrder->dicsountGcAmnt = number_format((float)$requ[7], 2, '.', '');
        ...
        if($requ[8] != 0){
            $TheGC = giftCard::find($requ[8]);
            if($TheGC != Null){
                $TheGC->gcSumInChfUsed = number_format($TheGC->gcSumInChfUsed + (float)$requ[7],2,'.','');
    ...
    public function oPayFStafPaySelectedRes(Request $req){
        ...
        $payload = array(
            ...
            'ReturnUrls' => array(
                'Success' => "https://demo.qrorpa.ch/oPayFStafPaySelectedResRegOrd/?OPR=".$OnlinePayRegister2,
                'Fail' => "https://demo.qrorpa.ch/?Res=".$oPForCl->resId."&t=".$oPForCl->tableNr
            ),
            'Notification' => array(
                'PayerEmail' => "erikthaqi1@amail.com",
    ...
    public function oPayFStafPaySelectedResRegOrd(Request $request){
        ...
        foreach(explode('||',$oPForCl->prodSelected) as $oneTabOrSel){ 
            $oneTabOrSel2d = explode('-8-',$oneTabOrSel);
            array_push($allMyOr,$oneTabOrSel2d[0]); 
        }
        ...
        foreach(TabOrder::whereIn('id',$allMyOr)->get() as $tOr){
            ...
            foreach(explode('||',$oPForCl->prodSelected) as $oneTabOrSel){ 
                $oneTabOrSel2D = explode('-8-',$oneTabOrSel);
                if($oneTabOrSel2D[0] == $tOr->id){
                    $thisTabSasisSel = $oneTabOrSel2D[1];
                }
            }

            if($thisTabSasisSel == $tOr->OrderSasia){
                ...
                $tOr->OrderSasia = $tOr->OrderSasia - $thisTabSasisSel;
                $tOr->OrderQmimi = number_format($tOr->OrderQmimi - $qmimiOfSelected, 2, '.', '');
                if($tOr->OrderSasiaDone >= $thisTabSasisSel){
                    $tOr->OrderSasiaDone = $tOr->OrderSasiaDone - $thisTabSasisSel;
                }
                $tOr->save();
                ...
                $newTabOrder->OrderSasia = $thisTabSasisSel;
                $newTabOrder->OrderSasiaDone = $thisTabSasisSel;
            ...
            if($tOr->status != 9 ){
                ...
                if($saveOrderAll != ''){
                    $saveOrderAll .= "---8---".$tOr->OrderEmri."-8-".$tOr->OrderPershkrimi."-8-".$tOr->OrderExtra.'-8-'.$thisTabSasisSel.'-8-'.$qmimiOfSelected
                    .'-8-'.$regType.'-8-'.$tOr->OrderKomenti.'-8-'.$tOr->prodId.'-8-'.$grId;
                }else{
                    $saveOrderAll = $tOr->OrderEmri."-8-".$tOr->OrderPershkrimi."-8-".$tOr->OrderExtra.'-8-'.$thisTabSasisSel.'-8-'.$qmimiOfSelected
                    .'-8-'.$regType.'-8-'.$tOr->OrderKomenti.'-8-'.$tOr->prodId.'-8-'.$grId;
                }
    ...
    public function oPayFStafPayTakeaway(Request $req){
        ...
        $payload = array(
            ...
            'ReturnUrls' => array(
                'Success' => "https://demo.qrorpa.ch/oPayFStafPayTakeawayRegOrd/?OPR=".$OnlinePayRegister2,
                'Fail' => "https://demo.qrorpa.ch/?Res=".$oPForCl->resId."&t=".$oPForCl->tableNr
            ),
            'Notification' => array(
                'PayerEmail' => "erikthaqi1@amail.com",
